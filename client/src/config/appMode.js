...
/** @typedef {'demo' | 'development' | 'production'} AppMode */
...
export function getRuntimeModeOverride() {
  ...
  try {
    ...
    if (raw === APP_MODE.DEMO || raw === APP_MODE.DEVELOPMENT || raw === APP_MODE.PRODUCTION) {
      return raw
    }
  } catch {
    /* ignore */
...
export function listAvailableAppModes() {
  return [APP_MODE.DEMO, APP_MODE.DEVELOPMENT, APP_MODE.PRODUCTION]
}
