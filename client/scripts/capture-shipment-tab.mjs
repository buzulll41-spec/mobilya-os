...
const outFile = path.join(outDir, 'shipment-tab-erp.png')
...
async function main() {
  ...
  if ((await page.locator('.mos').count()) === 0) {
    console.error('SCREENSHOT_FAIL: app shell not loaded')
    await browser.close()
    process.exit(1)
  }
  ...
  const shipmentTab = page.locator('.oop-tab', { hasText: 'Sevk' })
  if ((await shipmentTab.count()) > 0) {
    await shipmentTab.click()
  }
  await page.waitForSelector('.oop-shipment', { timeout: 10_000 })
  await page.waitForTimeout(500)

  const panel = page.locator('.oop-shipment')
